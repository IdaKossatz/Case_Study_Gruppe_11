...
Da für die Aufgabe Einzelteile und Komponenten aus dem Fahrzeug Typ 11, gebaut von der Firma OEM1, gefragt sind, werden anhand der ersten drei Tabellen sowie der jeweiligen Bestandteile_Komponente_KomponentenID Tabellen die restlichen benötigten Dateien rausgesucht. 
...
2. Gegebenenfalls Anpassen der Tabelle, so dass die Spalten jeder Komponenten- bzw. Einzelteiltabelle uniforme Namen und Datentypen haben. Das ist wichtig, damit die "Transform" Funktionen funktionieren.
```
...
In diesem Beispiel wird für eine Komponente nur ein Einzelteil eingelesen. Welche Einzelteile für welche Komponenten benötigt werden ist aus der Auflistung im Abschnitt "[Importieren der Daten]" zu entnehmen. In dem Fall von mehreren Einzelteilen pro Komponente erfolgt das Importieren und Einbinden analog. 
```
Einzelteil_T01_str <- readLines(paste(".\\Data\\Einzelteil\\Einzelteil_T01.txt"), warn=FALSE)
Einzelteil_T01_str <- str_replace_all(Einzelteil_T01_str, "[|]", "")
Einzelteil_T01_str <- str_replace_all(Einzelteil_T01_str, "[[:space:]]{3}", "\t")
...
13. Nun werden alle Tabelle, welche in Schritt 1 - 11 erstellt wurden, hintereinander in eine neue Tabelle zusammengefügt.
```
